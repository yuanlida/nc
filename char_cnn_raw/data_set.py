...
from .exe_config import EXEConfig
import random
import numpy as np
...
class Dataset(object):
    ...
    def _genTrainEvalData(self, x, y, word2idx, rate, char_ids):
        ...
        char_list = []
        for ids in char_ids:
            if len(ids) < self.config.sequenceLength:
                for i in range(self.config.sequenceLength - len(ids)):
                    ids.append([self.config._charToIndex[build_data.PAD]])
            elif len(ids) > self.config.sequenceLength:
                ids = ids[:self.config.sequenceLength]
            temp_ids = []
            for word in ids:
                if len(word) < self.config.word_length:
                    for i in range(self.config.word_length - len(word)):
                        word.append(self._charToIndex[build_data.PAD])
                elif len(word) > self.config.word_length:
                    word = word[:self.config.word_length]
    ...
    def _gen_char_vacablulary(self):
        ...
        self.config.char_size = len(self._indexToChar)
